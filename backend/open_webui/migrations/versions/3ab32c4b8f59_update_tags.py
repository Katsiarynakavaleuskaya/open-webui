...
def upgrade():
    ...
    log.debug(f"Primary Key: {existing_pk}")
    log.debug(f"Unique Constraints: {unique_constraints}")
    log.debug(f"Indexes: {existing_indexes}")
    ...
    with op.batch_alter_table("tag", schema=None) as batch_op:
        ...
        for constraint in unique_constraints:
            if (
                ...
                log.info(f"Dropping unique constraint: {constraint['name']}")
        ...
        for index in existing_indexes:
            if index["unique"]:
                if not any(
                    ...
                    log.info(f"Dropping unique index: {index['name']}")
